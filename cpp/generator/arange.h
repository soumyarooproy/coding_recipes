...
public:
  ...
  Arange(T start, T stop, T step = get_default_step<T>()):
      start_(start),
      step_(step),
      num_values_(get_num_values(start_, stop, step_)) {}

  auto begin() const {
    return iterator{start_, step_, 0};
  }

  auto end() const {
    return iterator{start_, step_, num_values_};
  }
...
public:
  ...
  ArangeIterator(T start, T step, size_t index) :
      start_(start), step_(step), index_(index) {}

  auto operator==(const ArangeIterator& other) const {
